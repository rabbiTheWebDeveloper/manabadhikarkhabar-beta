...
function PublicHeaderAndNav() {
  ...
  return (
    <>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 animate-fade-in flex justify-end no-print md:hidden" onClick={() => setMobileMenuOpen(false)}>
          <div className="w-[78%] max-w-[320px] bg-white h-full p-4 shadow-2xl flex flex-col gap-4 animate-slide-right overflow-y-auto safe-bottom" onClick={(e) => e.stopPropagation()}>
      ...
      {/* Top Bar for weather, date, social media icons — hidden on mobile for clean UX */}
      <div className="border-b border-gray-200 bg-white no-print hidden sm:block">
        <div className="max-w-7xl mx-auto px-4 py-1.5 flex justify-between items-center text-sm text-gray-700">
          <div className="flex items-center gap-4">
            <span className="font-bangla font-medium text-xs sm:text-sm">{banglaDate}</span>
            <WeatherWidget />
          </div>
          <div className="flex items-center gap-4">
      ...
      <header className="border-b-[3px] border-[#BC1E2D] py-2.5 sm:py-5 bg-white shadow-xs no-print">
      ...
      <nav className="border-b border-gray-200 sticky top-0 bg-white z-50 shadow-sm overflow-x-auto scrollbar-hide no-print">
        <div className="max-w-7xl mx-auto px-3 sm:px-4">
          <ul className="flex items-center gap-3 sm:gap-5 md:gap-6 text-[13px] sm:text-[14px] md:text-[16px] font-bold text-gray-800 py-2.5 sm:py-3.5 font-bangla whitespace-nowrap select-none">
            {categories.map(cat => (
              <li 
                key={cat.value}
                onClick={() => handleCategorySelect(cat.value)}
            ...
            <li className="ml-auto flex items-center gap-2.5">
              ...
              <Link 
                href="/submit-news" 
                className="hidden sm:flex items-center gap-1.5 bg-[#BC1E2D] text-white hover:bg-red-800 px-4 py-2 rounded-full text-xs font-extrabold transition-all shadow-sm hover:shadow active:scale-95 cursor-pointer"
              >
